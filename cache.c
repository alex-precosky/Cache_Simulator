...
{
    /* TO BE COMPLETED BY THE STUDENT */
  unsigned char* char_addr = cache_line->data + offset;
  int* int_addr = (int*)char_addr;
    return *(int_addr);
}
...
{
    /* TO BE COMPLETED BY THE STUDENT */
    for (int i = 0; i < cache->associativity; i++) { // for each cache line in the set... look for one that is not valid
    ...
    // no invalid cache lines, so we'll have to pick a victim. choose the least recently used.
...
//    printf("address: %x  block size %d block address %x source addr %x\n", address, cache->block_size, (void*)(address & ~cache->block_offset_mask), line->data);
...
int cache_read(cache_t *cache, int *address)
{
  ...
  intptr_t cache_index = ((intptr_t)address & (cache->cache_index_mask << cache->cache_index_shift)) >> cache->cache_index_shift;

  cache_set_t* cache_set = &(cache->sets[cache_index]);


  intptr_t block_index = (intptr_t)address & cache->block_offset_mask;

//  printf("address %x tag %x index %x cache_set %x block_index %x\n", address, tag, cache_index, cache_set, block_index);
  ...
  if(line)
    {
      // cache hit
      cache->access_count++;
      return cache_line_retrieve_data(line, block_index);
    }
  else
    {
      // cache miss
      cache->miss_count++;
      cache->access_count++;
      cache_set_add(cache, cache_set, (intptr_t)address, tag);
      return *address;
    }
